...
#       for box in bboxes:
#             vis_image = cv2.rectangle(vis_image, (int(box.x_min), int(box.y_min)), (int(box.x_max), int(box.y_max)), (255,0,0),2)
#       return vis_image

# def detect_image(image, detectors, detection_thresholds):
#       assert set(detectors.keys()) == set(detection_thresholds.keys()),\
...
# def test_detector(providers, input_path, output_path, weights_path,
#                   face_threshold, plate_threshold):
#       detector = {
...
#             vis_image = draw_bboxes(image, detected_boxes)
#             save_np_image(vis_image, output_path / name)
